...
// MARK: - Watch Hands Component (High-frequency updates)
struct WatchHandsView: View {
    ...
    // High-frequency timer only for hands
    let handsTimer = Timer.publish(every: 0.01, on: .main, in: .common).autoconnect()
    ...
    var body: some View {
        ...
        .onReceive(handsTimer) { input in
            currentTime = input
        }
        .onDisappear {
            handsTimer.upstream.connect().cancel()
        }
    }
    
    // MARK: - Hand Calculation Functions
...
// MARK: - Static Watch Elements Component (Low-frequency updates)
struct StaticWatchElementsView: View {
    let currentTime: Date
    let currentHour: Int
    let center: CGPoint
    let hourPositions: [Int: CGPoint]
    let hourAngles: [Int: Angle]
    ...
    // MARK: - Helper Functions for Static Elements
...
// MARK: - Main Watch View V2
struct AnalogWatchViewV2: View {
    @StateObject private var fontManager = CustomFontManager.shared
    @Environment(\.isLuminanceReduced) var isLuminanceReduced
    @EnvironmentObject var settings: AppSettings
    @EnvironmentObject var colorManager: ColorManager
    ...
    // Slower timer for date and current hour highlighting (updates once per second)
    let slowTimer = Timer.publish(every: 60.0, on: .main, in: .common).autoconnect()
    ...
    var body: some View {
        ZStack {
            ...
            GeometryReader { geometry in
                ...
                ZStack {
                    // Static elements (numbers, team name, barometer) - updates once per second
                    StaticWatchElementsView(
                    ...
                    // Dynamic hands with high-frequency updates - updates 100x per second
                    WatchHandsView()
                        .position(center)
                }
                .offset(y: -7)
            ...
            .onReceive(slowTimer) { input in
                // Only update time for elements that need less frequent updates
                currentTime = input
            }
        }
        .onDisappear {
            slowTimer.upstream.connect().cancel()
        }
    }
...
// MARK: - Preview
struct AnalogWatchViewV2_Previews: PreviewProvider {
